...
being the Liva Healthcare app which will record lifestyle information
such as meals and physical activity, and questionnaires/site visit forms
in REDCap. In addition connected devices (smart watch, smart scales,
blood pressure monitor) will transmit data directly to the Liva app.
...
Liva app: CSV exports via FTP to GenomeDK.\
- Watch, scales: Direct import into Liva app.\
...
Liva app data: Unknown.\
MyFood24 dietary logs: Unknown.\
Sensor data (SENS, Libre): Unknown.\
Scanner data:\
- VO2Max: 20KB times 1 reading per visit, 4 visits = 20x4x24 = 2MB.\
- Finapres: 60KB times 8 readings per visit, 6 visits = 60x8x6x24 =
69MB.
...
lifestyle data and connected device readings. In addition there will be
test results (like VO2Max and Finapres) which will be sent to GenomeDK
directly, the same applies to data from MyFood24.

GenomeDK's HPC facility will serve as the final repository for processed
and collated data, ensuring secure storage and computational capacity
for analysis.
...
Validation checks: Automated checks in REDCap to ensure completeness and
consistency of clinical data (Responsible: Data Architect).

Device data verification: Uploaded sensor data will be checked against
expected ranges and timestamps (Responsible: Site Staff using REDCap).

Monitoring for missing/anomalous data: Regular reports will identify
gaps or anomalies, with follow-up procedures for correction and will be
run during transfer to GenomeDK. (Responsible: Data Architect).
